...
function task2($log) // Добавляем для первого аргумента переменную
{
    ...
    foreach ($arr as $key => $value) {
        if (($key > 1) && ($value > 1)) { // Условие, по которому опериции с первым символом не проводим
            switch ($log) {
                case '-':
                    $score -= $value;
                    break;
...
function task3($i, $j)
{
    ...
    if ((gettype($i) == 'integer') && (gettype($j) == 'integer') && $i >= 1 && $j >= 1) { // проверяем на целые числа
        ...
        echo 'Введены не целые числа или 0';
    }
}
